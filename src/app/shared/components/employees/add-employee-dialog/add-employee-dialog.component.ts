...
@Component({
    ...
    <div class="dialog-container">
      <div class="d-flex align-items-center justify-content-between p-x-24 p-t-24 p-b-16">
        <div class="d-flex align-items-center">
          <div class="header-icon-box m-r-16 d-flex align-items-center justify-content-center bg-light-primary text-primary rounded">
            <i-tabler [name]="isEditMode ? 'user-edit' : 'user-plus'" class="icon-24"></i-tabler>
          </div>
          <div>
            <h2 class="mat-headline-6 f-w-600 m-b-4">{{ isEditMode ? 'Edit Employee' : 'Add New Employee' }}</h2>
            <span class="f-s-14 text-muted">{{ isEditMode ? 'Update details for the employee.' : 'Enter details for the new employee.' }}</span>
          </div>
        </div>
      ...
      <mat-dialog-content class="p-x-24 p-t-8 p-b-24 mat-typography">
        <form [formGroup]="employeeForm" class="employee-form">
          ...
          <div class="m-b-12">
            ...
            <div class="phone-input-group">
              ...
              <mat-form-field appearance="outline" subscriptSizing="dynamic" class="phone-number w-100 theme-input">
                ...
                <mat-error *ngIf="employeeForm.get('phone')?.hasError('minlength') || employeeForm.get('phone')?.hasError('maxlength')">
          ...
          <div class="row">
            <div class="col-sm-6 m-b-12">
              ...
              <mat-form-field appearance="outline" subscriptSizing="dynamic" class="w-100 theme-input">
                ...
                <mat-error *ngIf="employeeForm.get('branchId')?.hasError('required')">Branch is required</mat-error>
              </mat-form-field>
            </div>
            <div class="col-sm-6 m-b-12">
          ...
          <div class="m-b-12">
            <mat-label class="f-w-600 m-b-8 d-block text-dark">Manager</mat-label>
            <mat-form-field appearance="outline" subscriptSizing="dynamic" class="w-100 theme-input">
              ...
              <mat-hint *ngIf="!isLoadingManagers && managers.length === 0 && employeeForm.get('branchId')?.value">No managers found for this branch</mat-hint>
            </mat-form-field>
          </div>
        </form>
      </mat-dialog-content>
  ...
  styles: [`
    ...
    /* Angular Material Form Field Overrides for Modern Look */
    ::ng-deep .theme-input .mat-mdc-text-field-wrapper {
      background-color: transparent !important;
    }
    
    ::ng-deep .theme-input .mdc-notched-outline__leading,
    ::ng-deep .theme-input .mdc-notched-outline__notch,
    ::ng-deep .theme-input .mdc-notched-outline__trailing {
    ...
    :host-context(.dark-theme) {
      ...
      ::ng-deep .theme-input .mat-mdc-select-value-text {
        color: #f8fafc !important;
      }
    }
  `]
...
export class AddEmployeeDialogComponent implements OnInit {
  ...
  constructor(
    ...
    this.employeeForm = this.fb.group({
      ...
      // Default hidden properties as per the previous create JSON payload
      managerId: [null],
      isSenior: [false]
    });
  }
  ...
  ngOnInit(): void {
    if (this.data && this.data.id) {
      ...
      this.employeeForm.patchValue({
        ...this.data,
        roleId: this.data.role?.id || this.data.roleId,
        branchId: this.data.branch?.id || this.data.branchId,
    ...
    this.employeeForm.get('mobileCountryCodeId')?.valueChanges.subscribe(mccId => {
      const mcc = this.mobileCountryCodes.find(m => m.id === mccId);
      if (mcc && mcc.mobileNumberLength) {
        this.employeeForm.get('phone')?.setValidators([
          Validators.required,
    ...
    // When branch changes (user interaction), reset manager and fetch new ones
    this.employeeForm.get('branchId')?.valueChanges.subscribe(branchId => {
      this.managers = [];
      this.employeeForm.patchValue({ managerId: null }, { emitEvent: false });
    ...
    this.masterDataService.getRoles().subscribe({
      next: (res) => {
        if (res?.data) {
          ...
          const managerRole = this.roles.find(r => r.name?.toUpperCase() === 'MANAGER');
          if (managerRole) {
            this.managerRoleId = managerRole.id;
            // If branch is already selected (edit mode), load managers
            const currentBranch = this.employeeForm.get('branchId')?.value;
            if (currentBranch) {
    ...
    this.masterDataService.getMobileCountryCodes().subscribe({
      next: (res) => {
        if (res?.data) {
          ...
          if (!this.isEditMode || !this.employeeForm.get('mobileCountryCodeId')?.value) {
            const defaultMcc = this.mobileCountryCodes.find(m => m.mobileCode === '+91') || this.mobileCountryCodes[0];
            if (defaultMcc) {
              this.employeeForm.patchValue({ mobileCountryCodeId: defaultMcc.id });
            }
          }
        }
      },
  ...
  getSelectedMcc(): MobileCountryCode | undefined {
    const id = this.employeeForm.get('mobileCountryCodeId')?.value;
    ...
    if (this.isEditMode) {
      this.employeeService.updateEmployee(this.data.id, formValue).subscribe({
        next: (res) => {
          ...
          this.notificationService.showSuccessToast('Employee updated successfully.', 'Success');
          this.closeDialog(res); // Pass updated employee back
        },
        error: (err) => {
          this.isSubmitting = false;
          console.error('Error updating employee:', err);
          this.notificationService.showErrorToast('Failed to update employee.', 'Error');
        }
      });
    } else {
      this.employeeService.createEmployee(formValue).subscribe({
        next: (res) => {
          this.isSubmitting = false;
          this.notificationService.showSuccessToast('Employee created successfully.', 'Success');
          this.closeDialog(res); // Pass created employee back
        },
        error: (err) => {
          this.isSubmitting = false;
          console.error('Error creating employee:', err);
          this.notificationService.showErrorToast('Failed to create employee.', 'Error');
        }
      });
    }
  }
