...
(async () => {
  ...
  if (await cardViewBtn.count() > 0) {
    ...
    await page.waitForTimeout(1000);
    await page.screenshot({ path: path.join(outDir, 'leads-mobile-card-view.png') });
    console.log('Captured mobile card view screenshot');
  }

  // Test Dialog / Add Lead modal on mobile
  const addLeadBtn = page.locator('.mobile-fab, .desktop-add-btn, button:has-text("Add Lead")');
  if (await addLeadBtn.count() > 0) {
    await addLeadBtn.first().click();
    await page.waitForTimeout(1500);
    await page.screenshot({ path: path.join(outDir, 'leads-mobile-add-dialog.png') });
    console.log('Captured mobile add dialog screenshot');
  }

  fs.writeFileSync(path.join(outDir, 'leads-results.json'), JSON.stringify(results, null, 2));
  console.log('Done testing leads responsive!');
  await browser.close();
})();
